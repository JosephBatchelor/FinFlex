<?php
 ...
 if (isset($_SESSION['userId'])) {
  ...
  $sql4 = 'SELECT * FROM Balance WHERE Account_id = "'.$temp[$_COOKIE["test"]].'" AND DataToken = "'.$_SESSION['token'].'";';
      ...
      if ($resultCheck4 > 0) {
          ...
          while($row4 = mysqli_fetch_assoc($result4)){
            ...
            <script>
             ...
             function update(){
             myChart.data.datasets[0].data[1] = '.(float)$row4['available'].';
             myChart.data.datasets[0].data[2] = '.(float)$row4['current'].';
             myChart.data.datasets[0].data[3] = '.(float)$row4['overdraft'].';
             myChart.update();
            }
             update();
             </script>
